...
const CONFIG = {
  ...
  TWITTER_URL: "https://x.com/stacyapesol", // placeholder — update once the account exists
...
// ---------- Monkey's feed: duplicate items once so the CSS loop is seamless ----------
(function feedLoop() {
  const track = document.getElementById("feed-track");
  if (!track) return;
  const clone = track.innerHTML;
  track.innerHTML = clone + clone; // duplicate so translateY(-50%) loops without a visible seam
})();
...
// ---------- Chart-refresh counter: playful count-up ----------
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting && !done) {
          ...
          animateTo(4829); // arbitrary bit — this is a joke stat, not a real metric
          obs.disconnect();
        }
      });
    },
